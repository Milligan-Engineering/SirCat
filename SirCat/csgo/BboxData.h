...
public:
	BboxData() = default;
	BboxData(const wstring csvName) : Archive(csvName) {}; //Specify Archive constructor; acts like default constructor otherwise
	BboxData(const BboxData &otherBboxData) : Archive(otherBboxData, nullptr) {}; //Copy constructor; see above comment
	~BboxData() = default;
...
private:
	...
	//Precondition: applicationName contains the path to a valid .exe; *pPi is modifiable.
	//Postcondition: Returns true if the .exe was successfully started, and false otherwise.

	static BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam); //Boilerplate Windows callback function for EnumWindows
	static BOOL CALLBACK EnumChildProc(HWND hwnd, LPARAM lParam); //Boilerplate Windows callback function for EnumChildWindows
	...
	//Precondition: hFind, lpBuffer, *lpBuffer, lpFileName, *lpFileName, and FindFileData are modifiable.
